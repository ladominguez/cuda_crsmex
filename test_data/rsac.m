...
% DRLA START -> (next 11 lines) - Handles endian issue
if nargin <= 1 
...
if ( fid > 0 )
   ...
   st = fclose(fid);
   if s.nvhdf > 10 || s.nvhdf < 0
%	disp(['Endian problem found with file ' file ' trying to fix it.' ])
        s=rsac(file,'big-endian');
   end
else
